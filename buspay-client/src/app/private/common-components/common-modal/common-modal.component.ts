...
export class CommonModalComponent {
  ...
  ngOnInit() {
    this.modalSubscription = this.modalService.modalConfig$.subscribe(
      (config) => {
        if (config) {
  ...
  hideModal() {
    ...
    if (this.modalService.modalConfig$) {
      this.modalService.hideModal();
    }
  }
  resetModal() {
  ...
  setModalDimensions(width?: string, height?: string) {
    if (this.commonModal?.nativeElement) {
      const modalDialog =
        this.commonModal.nativeElement.querySelector('.modal-dialog');
      const modalContent =
        this.commonModal.nativeElement.querySelector('.modal-content');
